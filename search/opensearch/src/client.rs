...
impl OpenSearchApi {
    ...
    fn sign_request(&self, method: &str, path: &str, body: &str, date: &DateTime<Utc>) -> String {
        let service = "es";
    ...
    async fn request(&self, method: reqwest::Method, path: &str, body: Option<Value>) -> Result<Response, SearchError> {
        ...
        let now = Utc::now();
        let body_str = body.as_ref().map(|b| serde_json::to_string(b).unwrap()).unwrap_or_default();
        
        let authorization = self.sign_request(method.as_str(), path, &body_str, &now);
        
        let mut request = self.client.request(method, &url);
        request = request.header("Authorization", authorization);
        request = request.header("x-amz-date", now.format("%Y%m%dT%H%M%SZ").to_string());
        ...
        if response.status().is_success() {
            ...
            if let Ok(es_error) = serde_json::from_str::<ElasticSearchError>(&error_text) {
                match es_error.error.error_type.as_str() {
                    ...
                    _ => Err(SearchError::Internal(format!("OpenSearch error: {}", es_error.error.reason)))
                }
            } else {
    ...
    pub fn bulk_index(&self, index: &str, docs: Vec<EsDoc>) -> Result<(), SearchError> {
        ...
        rt.block_on(async {
            ...
            let now = Utc::now();
            let authorization = self.sign_request("POST", "/_bulk", &body, &now);
    ...
    pub fn update_mapping(&self, index: &str, mapping: Value) -> Result<(), SearchError> {
        ...
        rt.block_on(async {
            self.request(reqwest::Method::PUT, &path, Some(mapping)).await?;
